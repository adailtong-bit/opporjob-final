...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const categories = ['Technology', 'Marketing', 'Education', 'Services', 'Maintenance', 'Electronics', 'Furniture', 'Sports', 'Construction', 'Design'];
    ...
    for (let i = 0; i < BATCH_TOTAL; i++) {
      ...
      mockItems.push({
        ...
        title: `${originalCategory} Service - Ref ${i}`,
        description: `Specialist in ${originalCategory}. Exclusive service in ${location}. Quality and speed guaranteed, free quote. Reference: ${randomBatchId}-${i}`,
        price: price,
        location: location,
