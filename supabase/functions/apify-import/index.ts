...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!APIFY_KEY) {
      throw new Error("Chave APIFY_KEY não configurada nas variáveis de ambiente.");
    }
    ...
    if (payload.datasetId) {
      ...
      if (apifyRes.ok) {
        items = await apifyRes.json();
         ...
         throw new Error("Erro na integração com Apify. Verifique se o Dataset ID é válido.");
      }
    }
    ...
    if (items.length > 0) {
      ...
      return new Response(
        JSON.stringify({ 
          success: true, 
          count: totalInserted,
          message: "Integração Apify executada com sucesso usando variáveis de ambiente seguras."
        }),
        {
