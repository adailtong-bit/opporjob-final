...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const categories = ['Tecnologia', 'Marketing', 'Educação', 'Serviços', 'Manutenção', 'Eletrônicos', 'Móveis', 'Esportes', 'Construção', 'Design'];
    const cities = ['São Paulo, SP', 'Rio de Janeiro, RJ', 'Curitiba, PR', 'Belo Horizonte, MG', 'Brasília, DF', 'Porto Alegre, RS', 'Salvador, BA', 'Recife, BA', 'Fortaleza, PE', 'Manaus, AM'];
    
    const randomBatchId = crypto.randomUUID().split('-')[0];

    for (let i = 0; i < BATCH_TOTAL; i++) {
      const originalCategory = categories[Math.floor(Math.random() * categories.length)];
      const location = cities[Math.floor(Math.random() * cities.length)];
      const price = Math.floor(Math.random() * 2000) + 50;
      
      let mappedCategory = originalCategory;
      const rule = mappings.find(m => m.source.toLowerCase() === originalCategory.toLowerCase());
      if (rule) {
      ...
      mockItems.push({
        ...
        photos: [`https://img.usecurling.com/p/600/600?q=${encodeURIComponent(originalCategory.toLowerCase())}`]
      });
    }
