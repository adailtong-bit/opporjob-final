...
//     USING: ((EXISTS ( SELECT 1    FROM purchase_orders   WHERE ((purchase_orders.id = purchase_order_items.purchase_order_id) AND (purchase_orders.requester_id = auth.uid())))) OR (is_admin() = true))
...
//     USING: ((auth.uid() = requester_id) OR (is_admin() = true))
